...
export function HorizontalWorkflowGuide() {
    ...
    return (
        <div className="bg-white rounded-xl shadow-[0_1px_3px_0_rgba(0,0,0,0.1),0_1px_2px_0_rgba(0,0,0,0.06)] border border-slate-100 mt-8 overflow-hidden">
            <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-2">
                    <Lightbulb className="h-5 w-5 text-blue-500" />
                    <h3 className="font-bold text-slate-900">Workflow Guide</h3>
                </div>
                <div className="text-xs text-blue-600 bg-blue-50 px-3 py-1 rounded-full font-bold">
                    Scroll horizontally to read all steps →
                </div>
            </div>
                    ...
                    {steps.map((step) => {
                        ...
                        return (
                            <div key={step.id} className={cn(
                                "relative w-72 rounded-xl p-5 border transition-all flex flex-col gap-4",
                                isActive ? "bg-slate-50/80 border-slate-200 shadow-sm" : "bg-white border-slate-100"
                            )}>
                                {/* Number Bubble */}
                                <div className={cn(
                                    "flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border transition-colors z-10",
                                    isCompleted
                                        ? "bg-slate-100 border-slate-200 text-slate-400"
                                        : isActive
                                            ? cn("text-white shadow-sm ring-4", activeColorClass)
